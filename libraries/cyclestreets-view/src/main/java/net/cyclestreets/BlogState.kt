...
class BlogState {
    ...
    inner class CheckBlogTask(private val checkBlogContext: Context) : TimerTask() {
        override fun run() {
            ...
            setLastBlogUpdate(checkBlogContext, blog.mostRecent())
        }
    }
    ...
    fun readUpdate(context: Context) {
        prefs(context).edit()
    ...
    private fun setLastBlogUpdate(context: Context, update: String?) {
        prefs(context).edit()
